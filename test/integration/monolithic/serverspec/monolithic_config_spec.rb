...
describe 'Monolithic configuration' do
  ...
  it 'has the rabbit ssl certificates installed' do
    expect(file('/etc/rabbitmq/ssl/frontend.bcn.abiquo.com.crt')).to be_owned_by('rabbitmq')
    expect(file('/etc/rabbitmq/ssl/frontend.bcn.abiquo.com.key')).to be_owned_by('rabbitmq')
  end
